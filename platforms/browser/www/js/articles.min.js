...
const HOST='https://api.knife-museum.ru';class Articles{constructor(articleBlock,articlesBlock){this._articleBlock=articleBlock;this._articlesBlock=articlesBlock;this._view=new _articlesView2.default();this.addArticle();this.addArticles();}
async addArticle(link='/pages/api/v1/home'){link=HOST+link;let view=this._view;let container=this._articleBlock;let promise=await fetch(link);let article=await promise.json();container.innerHTML=view.renderArticle(article,false);}
async addArticles(link='/pages/api/v1/home/'){link=HOST+link;console.log(link);let view=this._view;let container=this._articlesBlock;let promise=await fetch(link);let data=await promise.json();let dataPager={count:data['count'],total:data['total'],index:data['index'],pages:data['pages']};container.innerHTML=view.renderArticles(data.items);container.innerHTML+=this.addPager(data.links,dataPager);}
